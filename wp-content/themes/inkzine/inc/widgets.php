...
class inkzine_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'inkzine_rp', // Base ID
			__('Inkzine Recent Posts', 'inkzine'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'inkzine' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'inkzine' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'inkzine' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','inkzine' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'inkzine' ); ?></label>
...
add_action( 'widgets_init', 'register_inkzine_widget' );  
function register_inkzine_widget() {  
    register_widget( 'Inkzine_Recent_Posts' );  
}  
